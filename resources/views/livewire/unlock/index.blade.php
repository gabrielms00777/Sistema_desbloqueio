                            ...
                            <div id="filterDropdown"
                                ...
                                <ul class="space-y-2 text-sm" aria-labelledby="filterDropdownButton">
                                    <li class="flex items-center">
                                        <input id="apple" type="checkbox" wire:model='emergency'
                                            class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        <label for="apple"
                                            class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">Não
                                            emergencial</label>
                                    </li>
                                    <li class="flex items-center">
                                        <input id="fitbit" type="checkbox" wire:model='not_emergency'
                                            class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        <label for="fitbit"
                                            class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">Emergencial</label>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Numero de serie</th>
                                <th scope="col" class="px-4 py-3">Nº Desbloqueios</th>
                                <th scope="col" class="px-4 py-3">Responsavel</th>
                                <th scope="col" class="px-4 py-3">CNPJ</th>
                                <th scope="col" class="px-4 py-3">Data/Hora</th>
                                <th scope="col" class="px-4 py-3">Tecnico</th>
                                <th scope="col" class="px-4 py-3">Obs</th>
                                <th scope="col" class="px-4 py-3">Emergencial</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($unlocks as $unlock)
                                <tr class="border-b dark:border-gray-700">
                                    <th scope="row"
                                        ...
                                        {{ $unlock->company }}</th>
                                    <td class="px-4 py-3">{{ $unlock->rep->serial_number }}</td>
                                    <td class="px-4 py-3">{{ $unlock->amount }}</td>
                                    <td class="px-4 py-3">{{ $unlock->client_name }}</td>
                                    <td class="px-4 py-3">{{ $unlock->cnpj }}</td>
                                    <td class="px-4 py-3">{{ date_format($unlock->created_at, 'd/m/Y H:i:s') }}</td>
                                    <td class="px-4 py-3">{{ $unlock->responsible }}</td>
                                    <td class="px-4 py-3">{{ $unlock->observation }}</td>
                                    <td class="px-4 py-3">{{ $unlock->emergency ? 'Sim' : 'Não' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
